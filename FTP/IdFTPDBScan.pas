...
type
  ...
  private
    ...
    procedure _DeleteDirFTP(const Dir: string);
    procedure _DeleteDir(const Dir: string);
    procedure _GetFiles(const P: string; var LF, LD: TStringList);
...
procedure TIdFTPDBScan._DeleteDirFTP(const Dir: string);
var     
  n,m,t,nomer: Integer;
begin
  m:=-1;
...
procedure TIdFTPDBScan._DeleteDir(Const Dir: string);
var
  n,m,t,nomer: Integer;
begin
  m:=-1;
...
procedure TIdFTPDBScan._GetFiles(const P: string; var LF, LD: TStringList);
var
  TempPath: String;
  LT:       TStringList;
  S:        TSearchRec;
...
begin
  ...
  2:if(not IBQGet1.Eof)and(IBQGet1.Database.Connected)then
    try
      ...
      try
        Delete(IBQGet1.FieldValues['v_path']);
      except
      end;
    finally
      IBQGet1.Next;
...
begin
  ...
  2:if(not IBQGet2.Eof)and(IBQGet2.Database.Connected)then
    try
      ...
      try
        _DeleteDirFTP(IBQGet2.FieldValues['v_path']);
      except
      end;
    finally
      IBQGet2.Next;
