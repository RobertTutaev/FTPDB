...
type
  TImageType=(itUnknown,itBitmap,itJPG,itICO);
...
begin
   ...
   try
    ...
    if (sign[0]='B') and (sign[1]='M') then
     Result:=itBitmap
    else
    if JpegSign=sign then
     Result:=itJPG
    else
    begin
     Stream.Position:=0;
     Stream.ReadBuffer(CI, SizeOf(CI));
     if (CI.wType in [RC3_STOCKICON, RC3_ICON]) then
      Result:=itICO
    end;
   finally
...
//          DrawPict.Assign(FDataLink.Field);
        ...
        if Stretch then
          ...
          begin
            ...
            if DrawPict.Graphic.Width<>0 then
             proporciaImage:=DrawPict.Graphic.Height/DrawPict.Graphic.Width
            else
             proporciaImage:=0;
            if Width<>0 then
             proporciaSelf:=
              (stretchRect.Bottom-stretchRect.Top)/(stretchRect.Right-stretchRect.Left)
            else
             proporciaSelf:=0;
            if (proporciaSelf<>0) and (proporciaImage<>0) then
            begin
              if proporciaSelf>proporciaImage then
              begin
                //За счет высоты
                stretchRect.Bottom:=stretchRect.Top+
                 Trunc((stretchRect.Right-stretchRect.Left)*proporciaImage);
                delta:=((ClientRect.Bottom-ClientRect.Top) -
                  (stretchRect.Bottom-stretchRect.Top)) div 2;
                Inc(stretchRect.Top,Delta);
                Inc(stretchRect.Bottom,Delta);
                FillRect(Rect(ClientRect.Left,ClientRect.Top,
                 ClientRect.Right,StretchRect.Top
                ));
                FillRect(Rect(ClientRect.Left,StretchRect.Bottom,
              ...
              begin
                ...
                delta:=((ClientRect.Right-ClientRect.Left) -
                  (stretchRect.Right-stretchRect.Left)) div 2;
                Inc(stretchRect.Right,Delta);
                Inc(stretchRect.Left,Delta);
                FillRect(Rect(ClientRect.Left,ClientRect.Top,
                 StretchRect.Left,ClientRect.Bottom
                ));
                FillRect(Rect(StretchRect.Right,ClientRect.Top,
                 ClientRect.Right,ClientRect.Bottom
                ));
              end;
...
procedure TpDBImage.Notification(AComponent: TComponent;
...
begin
  ...
  if (not FPictureLoaded or Reload) and FDataLink.Field.IsBlob then
   begin
     if (not FDataLink.Field.IsBlob and FDataLink.Field.IsNull)
      or (TBlobField(FDataLink.Field).BlobSize=0) then     
