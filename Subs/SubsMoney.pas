...
{      Переработка модуля Антона Близнец (1999г.)       }
...
Function RealToRouble(c: Extended; ShortStr: Boolean=False):String;
Function RealToRoub(c: Extended):String;
...
function RealToRouble(c: Extended; ShortStr: Boolean=False):String;
var
  rub:String[MaxPosition+3]; kop:String[2];
Const
...
begin{Возвращает число прописью с рублями и копейками}
  ...
  begin
    ...
    if ShortStr=False
    then Result:=NumToStr(rub)+'рубл'+ruble[ending(rub)]+' '+kop+' копе'+Kopeek[ending(kop)]
    else Result:=NumToStr(rub)+'руб.'+' '+kop+' коп.';
...
Function RealToRoub(c: Extended):String;
 ...
 function Add(AValue: Extended): String;
 begin
   Result:=FloatToStrF(Frac(c)*100, ffNumber, 10,0)
 end;

var i: Integer;
begin
  Result:=FloatToStr(Int(c))+' руб. '+Add(c)+' коп.'
end;
